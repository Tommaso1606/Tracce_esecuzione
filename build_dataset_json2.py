    ...
    with open('log-all-in-one.json', 'r') as file:
        ...
        for line in file:
            ...
            if line:
                ...
                #match = re.search(pattern, log_date)

                #if re.search(pattern1, log_date):
                    #log_date += '.000001'
                    #log_date_date = datetime.strptime(log_date, "%Y-%m-%d %H:%M:%S")
                #elif re.search(pattern2, log_date): 
